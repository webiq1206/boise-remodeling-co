...
Inspect every proposed SQL statement even when summary flags report no structural data loss and removal arrays are empty.

**Why:** A read-only schema comparison returned explicit DROP TABLE CASCADE statements while its structural-data-loss flag was false and tables-to-remove list was empty.

**How to apply:** Treat destructive SQL as a blocker regardless of summary metadata. Report it without applying changes when the user has authorized release synchronization only.