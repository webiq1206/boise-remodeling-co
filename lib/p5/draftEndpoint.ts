...
export async function getDraft(request:Request){try{protectRequest(request);const {id,key}=draftCredentials(request);const draft=await readDraft(id,key);
  ...
  return json({draft,...(stored?.customer_estimate?{result:customerPresentation(stored.customer_estimate)}:{}),...(withEvents?{events:(await draftEvents(id)).map(e=>({at:e.createdAt,kind:e.kind,stage:e.stage,file:e.file,provider:e.provider,model:e.model,status:e.status,code:e.code,message:e.message,durationMs:e.durationMs,attempt:e.attempt,fallback:e.fallback,outcome:e.outcome}))}:{})});}catch(error){return failed(error);}}
