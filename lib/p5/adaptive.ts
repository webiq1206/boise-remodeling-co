...
function pricingQuestionFields(answers:ScopeAnswers):ScopeField[]{
  ...
  if(builds.includes(service)){if(!answers.garageIncluded?.trim())required.push('garageIncluded');if(answers.garageIncluded==='yes'&&!answers.garageSqft?.trim())required.push('garageSqft');}
  ...
  if(scope){
    ...
    if(/cabinet|vanit(?:y|ies)/.test(scope)){if(!answers.cabinetBaseLf?.trim())required.push('cabinetBaseLf');if(!answers.cabinetUpperLf?.trim())required.push('cabinetUpperLf');if(!answers.cabinetTallLf?.trim())required.push('cabinetTallLf');}
...
export function materialScopeFields(answers:ScopeAnswers,pricedFields:ScopeField[]=[]):ScopeField[]{
  const service=answers.service;const required=pricingQuestionFields(answers);
...
function choiceValues(field:ScopeField,answers:ScopeAnswers){const definition=SCOPE_FIELDS[field];if(definition.kind!=='choice')return undefined;if(field==='finish')return finishOptionsForService(answers.service);return definition.options.filter(v=>field!=='service'||(ESTIMATOR_BRAND.services as readonly string[]).includes(v));}
export function questionForField(field:ScopeField,answers:ScopeAnswers):ScopeQuestion{const values=choiceValues(field,answers);return {field,label:SCOPE_FIELDS[field].label,reason:questionReason(field,answers),...(values?.length?{values}:{})};}
export function scopeQuestions(input:ScopeAnswers,extraction:ScopeExtraction|null,conflicts:ScopeConflict[]=[],skipped:ScopeField[]=[],pricedFields:ScopeField[]=[]):ScopeQuestion[]{
  const answers=deriveScopeAnswers(input);const relevant=new Set<ScopeField>(['service',...materialScopeFields(answers,pricedFields),...pricedFields]);if((extraction?.takeoffs?.length||0)>0)relevant.delete('taskList');
...
export function scopeAssumptions(answers:ScopeAnswers,skipped:ScopeField[]=[]){
  const notes:string[]=[];if(!answers.location&&!answers.address)notes.push('General service-area pricing; location, access and jurisdiction will be confirmed.');if(!answers.urgency)notes.push('Standard scheduling; priority or emergency work is not included.');if(!answers.finish&&finishServices.includes(answers.service||''))notes.push('Finish level not chosen; standard finishes are assumed until you select one.');if(answers.length&&answers.width&&answers.sqft&&sameAnswer('sqft',answers.sqft,deriveScopeAnswers({...answers,sqft:''}).sqft||'0'))notes.push(`Project area calculated from ${answers.length} × ${answers.width} feet. Confirm irregular areas during the site visit.`);for(const k of skipped)if(!answers[k])notes.push(`${SCOPE_FIELDS[k].label}: not yet known; requires an allowance or pricing review.`);return notes;
}
...
export function scopeQuestionsForBrand(...args:Parameters<typeof scopeQuestions>):ScopeQuestion[]{
 ...
 if(service&&!service.startsWith('cabinet-')){const next=[...args] as Parameters<typeof scopeQuestions>;next[2]=(args[2]||[]).filter(conflict=>conflict.field!=='cabinetRoom');if(args[1])next[1]={...args[1],conflicts:args[1].conflicts.filter(conflict=>conflict.field!=='cabinetRoom')};return scopeQuestions(...next).filter(question=>question.field!=='cabinetRoom');}
