...
export function P5Estimator({defaultService='',headingAs='h1',projectSource,layout='embedded',onExit}:P5EstimatorProps){
  ...
  const send=()=>{
    ...
    if(composerMode==='answer'&&active){
      ...
      if(definition.kind==='number'){const issue=validateScopeAnswer(active.field,text);if(!issue){answer(active.field,text);logExchange(active,text);void advance();return;}addToProject(contextualCustomerAnswer(active.reason,text));return;}
  ...
  const deliveryState=customerDelivery?.status==='sent'?'sent':customerDelivery?.status==='needs-review'?'review':'pending';
  ...
  const pausedCard=paused&&<section className={styles.notice} role="status" aria-live="polite"><h3>{paused.kind==='analysis'?'Reviewing your project':'Still preparing your estimate'}</h3><p>{paused.processing?.totalPages?`${Math.min(paused.processing.totalPages,paused.processing.readPages||0)} of ${paused.processing.totalPages} pages are checked so far. `:''}This is taking longer than usual. Your completed work is saved. This page checks for progress while open and shows the result when it is ready.</p><div className={styles.actions}><button type="button" className={styles.primary} onClick={continuePaused}>Keep going</button><button type="button" className={styles.ghost} onClick={()=>setPaused(null)}>Come back later</button></div></section>;
  ...
  const questionStage=draft.step===1&&!busy&&!preparingFiles&&<Message role="assistant">{!active&&warningCard}{active?<section key={active.instructionId||active.field} ref={el=>{stageRef.current=el;}} className={styles.question} aria-label="Project question"><p className={styles.eyebrow}>{active.label}</p><h2 tabIndex={-1} data-stage-heading>{active.reason}</h2>{active.detail&&(active.handoff?<p className={styles.hint}>{active.detail}</p>:<details className={styles.context}><summary className={styles.hint}>Why we ask</summary><p className={styles.hint}>{active.detail}</p></details>)}{active.handoff&&<div className={styles.actions}><a className={styles.primary} href={active.handoff.url}>{active.handoff.label}</a></div>}{warningCard}{active.values?.length?<div className={styles.choices} role="group" aria-label="Suggested answers">{active.values.map(value=><button type="button" className={styles.choice} key={value} onClick={()=>choose(value)} aria-pressed={reply===optionLabel(value)||reply.startsWith(optionLabel(value)+'\n')}>{optionLabel(value)}</button>)}</div>:null}{alertCard}{!active.handoff&&<div className={styles.questionActions}>{active.field!=='service'&&!active.conflict&&!active.instructionId&&<button type="button" className={styles.secondary} onClick={skipQuestion}>Not sure yet</button>}<span className={styles.hint}>{active.values?.length?'Choose an option or type your answer. Add details if needed, then send.':'Type your answer below and send it.'}</span></div>}</section>:<div ref={el=>{stageRef.current=el;}} className={styles.card}><h3 tabIndex={-1} data-stage-heading>Your details are complete.</h3><p className={styles.hint}>Review your project and add where to send your estimate.</p><div className={styles.actions}><button className={styles.primary} type="button" onClick={()=>{engage();showQuestions(current.current!);}}>Review your project <span aria-hidden="true">→</span></button></div></div>}</Message>;
