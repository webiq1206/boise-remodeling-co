...
import {deriveScopeAnswers,finishOptionsForService,finishServices,questionForField,scopeQuestionsForBrand as scopeQuestions,scopeAssumptions,validateScopeAnswer,type ScopeQuestion} from '@/lib/p5/adaptive';
...
export function P5Estimator({defaultService='',headingAs='h1',projectSource,layout='embedded',onExit}:P5EstimatorProps){
  ...
  const choose=(value:string)=>{if(!active||busyRef.current)return;engage();setError('');if(active.instructionId){setReply(value);change({pendingReply:{id:active.instructionId,answer:value}});logExchange(active,value);void advance({clarification:value});return;}answer(active.field,value);logExchange(active,readable(active.field,value));void advance();};
  ...
  async function submit(event:React.FormEvent){
    ...
    await run('Preparing your estimate...',async()=>{
      ...
      catch(failure){const details=(failure as any)?.details;if(details?.pricingReviewRequired){setMissingFields(parseMissing(details.missingFields));setVerificationItems(parseItems(details.verificationItems));throw new Error(details.error||'A few more details are needed before pricing.');}throw failure;}
  ...
  const send=()=>{
    ...
    if(composerMode==='answer'&&active){
      ...
      if(definition.kind==='choice'){const match=coerceChoice(active.field,text)||active.values?.find(v=>v.toLowerCase()===text.toLowerCase()||readable(active.field,v).toLowerCase()===text.toLowerCase());if(match){answer(active.field,match);logExchange(active,readable(active.field,match));void advance();return;}if(text.length>40||filesRef.current.length){addToProject(text);return;}setError('Choose one of the options above, or describe the change to your project in a full sentence.');return;}
      if(definition.kind==='number'){const issue=validateScopeAnswer(active.field,text);if(!issue){answer(active.field,text);logExchange(active,text);void advance();return;}if(text.length>40||filesRef.current.length){addToProject(text);return;}setError(issue);return;}
  ...
  useLayoutEffect(()=>{
    if(layout==='embedded'&&!expanded)return;
  ...
  },[stageKey,working,Boolean(draft),layout,expanded]);
  ...
  const assumptions=scopeAssumptions(draft.answers,draft.wizard?.skipped);const lastUserIndex=transcript.map(e=>e.role).lastIndexOf('user');
  ...
  const pausedCard=paused&&<section className={styles.notice} role="status" aria-live="polite"><h3>{paused.kind==='analysis'?'Still reading your project':'Still preparing your estimate'}</h3><p>{paused.processing?.totalPages?`${Math.min(paused.processing.totalPages,paused.processing.readPages||0)} of ${paused.processing.totalPages} pages are checked so far. `:''}This is taking longer than usual. Your completed work is saved and processing continues in the background; this page checks every few seconds and shows the result as soon as it is ready.</p><div className={styles.actions}><button type="button" className={styles.primary} onClick={continuePaused}>Keep going</button><button type="button" className={styles.ghost} onClick={()=>setPaused(null)}>Come back later</button></div></section>;
  const alertCard=error&&<div id={submitErrorId} className={styles.alert} role="alert" aria-live="assertive">{missingFields.length>0||verificationItems.length>0?<><h3>A few more details are needed</h3><p>{error}</p>{verificationItems.length>0&&<ul>{verificationItems.map(item=><li key={item}>{item}</li>)}</ul>}{missingFields.length>0&&<ul className={styles.missingList}>{missingFields.map(m=><li key={m.field}><button type="button" className={styles.secondary} onClick={()=>jumpToField(m.field)}><span>{m.label}</span><span aria-hidden="true">→</span></button></li>)}</ul>}</>:<p>{error}</p>}</div>;
  ...
  const resultStage=result&&<Message role="assistant"><div className={styles.stageHeading} ref={el=>{stageRef.current=el;}}><h2 tabIndex={-1} data-stage-heading>Your project estimate</h2><p className={styles.lead}>Your planning range and project scope are organized below.</p></div><div className={styles.rangeCard}><p className={styles.eyebrow}>{result.range?'Preliminary planning range':'Status'}</p><h2>{result.range?`${result.range.low.toLocaleString("en-US",{style:"currency",currency:"USD",maximumFractionDigits:0})} to ${result.range.high.toLocaleString("en-US",{style:"currency",currency:"USD",maximumFractionDigits:0})}`:"Your scope is ready for pricing review"}</h2><p>{result.message}</p></div><p className={styles.delivery} role="status" data-state={deliveryState}>{deliveryState==='sent'?`Your estimate was emailed to ${draft.contact.email} and the team has your record.`:deliveryState==='review'?'Your estimate is saved. The email could not be delivered automatically, so the team will check it and follow up. You do not need to submit again.':customerDelivery?`Your estimate is saved. We are sending a copy to ${draft.contact.email}.`:'Your estimate is saved. We are sending a copy to your email.'}</p><details className={styles.accordion} open><summary><span className={styles.accordionTitle}>Your estimate PDF</span><span className={styles.badge} data-kind="included">Ready</span></summary><div className={styles.accordionBody}><p className={styles.hint} style={{marginBottom:12}}>Your branded PDF includes the planning range, scope, inclusions, exclusions, allowances and assumptions shown here.</p><button type="button" className={styles.secondary} onClick={downloadPdf} disabled={locked}><FileGlyph/> Download estimate PDF</button></div></details><P5EstimateDetails result={result}/><details className={styles.accordion}><summary><span className={styles.accordionTitle}>Recommended next step</span></summary><div className={styles.accordionBody}><p>{result.nextStep}</p><div className={styles.resultActions}><a className={styles.primary} href={brand.consultationPath} onClick={()=>trackScopeEvent("onsiteRequested",draft.answers.service)}>Schedule a consultation</a><a className={styles.secondary} href="tel:+12084771169">Call {brand.phone}</a></div></div></details><p className={styles.disclaimer}>{result.disclaimer}</p>{alertCard}<div className={styles.actions}><button type="button" className={styles.ghost} onClick={()=>void switchProject()}>Start another project</button></div></Message>;
